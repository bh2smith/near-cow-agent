...
describe("Order Cancellation Route", () => {
  ...
  it.only("Test Cancel Dual Routed Logic", async () => {
    const chainId = 1;
    const orderUid =
      "0xe46e8c19e371fbcc3797898796292cba68eb8cc27ca32d35305e0087d8d9c46ab00b4c1e371dee4f6f32072641430656d3f7c0646899c71e";
    const signature =
    ...
    const getReq = new NextRequest(
      `http://localhost/api?chainId=${chainId}&orderUid=${orderUid}`,
      { method: "GET" },
    );
    const cancellationData = await logic(getReq);
    ...
    await expect(logic(postReq)).rejects.toThrow("AlreadyCancelled");
  });
});
