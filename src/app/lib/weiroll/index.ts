 ...
 * @returns An EVM call
 */
export function createWeirollDelegateCall(
  addToPlanner: (planner: WeirollPlanner) => void,
): EvmCall {
...
export function createWeirollContract(
  contract: EthersContract,
  commandflags?: WeirollCommandFlags,
): WeirollContract {
  return WeirollContract.createContract(
    contract,
    commandflags as unknown as Parameters<
      typeof WeirollContract.createContract
    >[1],
  );
}

export function createWeirollLibrary(
  contract: EthersContract,
): WeirollContract {
